...
def run_episode():
    ...
    print(f"[START] task=resolve_ticket env=openenv model={MODEL_NAME}")
    ...
    try:
        while not done:
            ...
            print(f"[STEP] step={step} action={action_text} reward={reward:.2f} done={str(done).lower()} error={error_str}")
